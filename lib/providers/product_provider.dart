...
class ProductProvider with ChangeNotifier {
    ...
    try {
      ...
      if (snapshot.exists) {
        ...
        _categories = data.values
            .map((item) => app_models.Category.fromJson(Map<String, dynamic>.from(item)))
            .toList();
      }
      notifyListeners();
    } catch (e) {
  ...
  List<Product> searchProducts(String query) {
    ...
    return _products.where((product) {
      return (product.name?.toLowerCase().contains(lowercaseQuery) ?? false) ||
             ...
             (product.categoryName?.toLowerCase().contains(lowercaseQuery) ?? false);
    }).toList();
  }
  ...
  Future<void> addProduct(Product product) async {
    try {
      ...
      await _firebaseService.getProductDetailRef(newId).set(productWithId.toJson());
  ...
  Future<void> updateProduct(Product product) async {
    try {
      await _firebaseService.getProductDetailRef(product.id).set(product.toJson());
      await loadProducts(); // Reload to get updated list
    } catch (e) {
  ...
  Future<void> addCategory(app_models.Category category) async {
    try {
      ...
      await _firebaseService.getCategoryDetailRef(newId).set(categoryWithId.toJson());
  ...
  Future<void> updateCategory(app_models.Category category) async {
    try {
      await _firebaseService.getCategoryDetailRef(category.id).set(category.toJson());
      await loadCategories(); // Reload to get updated list
    } catch (e) {
  ...
  Future<int> _getNextProductId() async {
    try {
      ...
      if (snapshot.exists) {
        final data = snapshot.value as Map<dynamic, dynamic>;
        final maxId = data.keys.map((e) => int.tryParse(e.toString()) ?? 0).fold(0, (a, b) => a > b ? a : b);
        return maxId + 1;
      }
      return 1;
    } catch (e) {
